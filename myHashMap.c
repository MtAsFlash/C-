...
MyHashMap *createMyHashMap(int(*hashCode)(void *key),
  ...
  re->hashCode = hashCode;//跳过
  re->equal = equal;//跳过
  for (int i = 0; i < re->initialCapacity; i++) {
    re->entryList[i] = createMyList();//创建链表
  }
  return re;
}
...
//是否包含某个key
//判断某Map中是否已经包含key值,调用listContainsEntry函数实现检测重复
int myHashMapContainsKey(MyHashMap * const map, void * const key) {
  int hasCode = (*(map->hashCode))(key);//获取到该数据的HashCode值
  hasCode %= map->initialCapacity;
  if (hasCode < 0)
    hasCode += map->initialCapacity;
  Entry * re = listContainsEntry(map->entryList[hasCode], key, map->equal);//检测是否已经出现key值
  return re != NULL;
}

//增加一条映射
//向Map集合中增加新的映射(key-value值)
void myHashMapPutData(MyHashMap * const map, void * const key,
  ...
  if (re == NULL) {
    ...
    //如果已经存在该key,修改原先的value为新要添加的value?
    re->value = value;
  }
}
...
Entry* myHashMapEntryIteratorNext(MyHashMapEntryIterator* iterator) {
  ...
  Entry * re = (Entry *) iterator->current->data;//取出Entry的值
  iterator->current = iterator->current->next;//指针移动
  return re;
}
